...
import marshmalliow.annotations.TextFile;

@SupportedAnnotationTypes("marshmalliow.annotations.TextFile")
@SupportedSourceVersion(SourceVersion.RELEASE_18)
@AutoService(javax.annotation.processing.Processor.class)
public class TextFileProcessor extends AbstractProcessor {

	@Override
	public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
		for(final Element element : roundEnv.getElementsAnnotatedWith(TextFile.class)) {
	...
	private void generateBuilderClass(TypeElement classElement) {
		...
		builderClassContent.append("import marshmalliow.core.file.text.AbstractTextFileBuilder;\n\n");
		builderClassContent.append("import marshmalliow.core.file.FileType;\n\n");
		
		builderClassContent.append("public class ").append(builderClassName).append(" extends AbstractTextFileBuilder<").append(className).append("> {\n");
		...
		builderClassContent.append("        if(!n.toLowerCase().endsWith(FileType.PLAIN_TEXT.getExtension())) n += FileType.PLAIN_TEXT.getExtension();\n");

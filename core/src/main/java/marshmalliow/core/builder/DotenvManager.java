...
public class DotenvManager {
	 ...
	 * 
	 * @param dir The directory where the .env file is located
	...
	public void addEnvFile(Directory dir, String fileName) throws IOException {
		try {
			...
			if(!dir.exists(fullName)) throw new FileNotFoundException();
			
			try (final BufferedReader br = new BufferedReader(new InputStreamReader(new BufferedInputStream(dir.openInputStream(fullName))))) {
				
				final DotenvParser parser = new DotenvParser(new DotenvLexer(br));

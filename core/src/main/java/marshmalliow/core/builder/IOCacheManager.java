...
//	private static final Logger LOGGER = LogManager.getLogger(IOCacheManager.class);
	...
	private final Map<String, AbstractFile> files = new HashMap<>();
	...
	/**
	 * Add a new Input/Output object extending {@link AbstractFile} into the cache.<br/>
	 * This method uses the object name (not the path) as the identifier. If you wish to use a specified id see {@link IOCacheManager#add(String, AbstractFile)}.
	...
	/**
	 * Add a new Input/Output object extending {@link AbstractFile} into the cache.<br/>
	 * If you wish to use the file's name as the identifier, see {@link IOCacheManager#add(String, AbstractFile)}.
	 * @param identifier The id used to register the file into the cache
	 * @param obj The object to be added
	 * @throws IdentifierAlreadyUsedException if a file is registered with the same identifier
	 */
	public void add(String identifier, AbstractFile obj) {
		try {
			LOCK.writeLock().lock();
	 ...
	 * This method uses the object name (not the path) as the identifier. If you wish to use a specified id see {@link IOCacheManager#replace(String, AbstractFile)}.
	 * @param obj The object to be added
	 */
	public void replace(AbstractFile obj) {
		try {
			LOCK.writeLock().lock();
	 ...
	 * Add a new Input/Output object extending {@link AbstractFile} into the cache.<br/>
	 * If an object is already using the same identifier, it will be erased.<br/>
	 * If you wish to use the file's name as the identifier, see {@link IOCacheManager#replace(AbstractFile)}.
	 * @param identifier The id used to register the file into the cache
	 * @param obj The object to be added
	 * @throws IdentifierAlreadyUsedException if a file is registered with the same identifier
	 */
	public void replace(String identifier, AbstractFile obj) {
		try {
			LOCK.writeLock().lock();
	 ...
	 * @param obj The {@link AbstractFile} object
	 * @return If the object has been successfully removed
	 */
	public boolean remove(AbstractFile obj) {
		try {
			LOCK.writeLock().lock();
	 ...
	 * @return If the object has been successfully removed
	 */
	public boolean remove(String identifier, AbstractFile obj) {
		try {
			LOCK.writeLock().lock();
	 ...
	 * @param identifier The id requested for the {@link AbstractFile} object
	 * @return the object corresponding to the id
	 * @throws FileNotLoadedException if no file is found with the provided identifier
	 * @see IOCacheManager#getAsOptional(String)
	 */
	public AbstractFile get(String identifier) {
		try {
			LOCK.readLock().lock();
			final AbstractFile result = this.files.get(identifier);
	 ...
	 */
	public Optional<AbstractFile> getAsOptional(String identifier) {
		try {
			LOCK.readLock().lock();
	 ...
	 * @param <E> A class extending {@link AbstractFile}
	 ...
	 */
	public <E extends AbstractFile> E get(String identifier, Class<E> castingClass) {
		try {
			LOCK.readLock().lock();
	 ...
	 */
	public <E extends AbstractFile> Optional<E> getAsOptional(String identifier, Class<E> castingClass) {
		try {
			LOCK.readLock().lock();
